...
<div id="pha-che-grid" class="d-flex flex-column gap-3"></div>
...
(() => {
    ...
    const fetchOrders = async () => {
        const response = await fetch(@json(route('pha-che.data')));
    ...
    grid.addEventListener('click', async event => {
        ...
        if (event.target.classList.contains('complete-item')) {
            ...
            try {
                const response = await fetch(@json(url('pha-che')) + '/' + id, {
                    method: 'PATCH',
                    headers: {
